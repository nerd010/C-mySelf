#include <stdio.h>
#include <stdlib.h> //为 rand()和srand ()函数提供原型
#include <time.h> //为time () 函数提供原型
#include "queue.h" //改变Item 的typedef
...
int main (void)
{
    ...
    long wait_time = 0; /* 从当前到 Sigmund 空闲所需的时间 */
    double min_per_cust; /* 顾客到来的平均间隔时间 */
    long line_wait = 0; /* 队列累计等待时间 */

    InitializeQueue (&line);
    srand (time (0));/* 随机初始化 rand() 函数 */
